...
def api_generate():
    ...
    try:
        ...
        # التحقق من أن الاستجابة JSON وليست صفحة خطأ HTML
        ...
        if "application/json" in content_type:
            ...
            if response.status_code == 200:
                ...
                return jsonify({"error": f"Gemini API Error: {res_json}"}), response.status_code
        else:
            return jsonify({"error": "Received non-JSON response from AI provider (High Demand/503)"}), 503
...
async def pitch_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if res.status_code == 200 and "application/json" in res.headers.get("Content-Type", ""):
            ...
            await update.message.reply_text("❌ حدث خطأ أثناء الاتصال بمحرك الذكاء الاصطناعي (ضغط عالٍ مؤقت).")
    except Exception as e:
